...
from utils import gif75speaker_res
import numpy as np
...
parser.add_argument("--epoch", type=int, default=50000, help="Number of epoch for training (default 50000)")
...
unet2 = Unet3D(dim = 64, dim_mults = (1, 2, 4, 8), lowres_cond=True).cuda()
...
dataset_75speaker = gif75speaker_res(image_path = opt.gif_path, img_per_gif=10, audio_path = opt.audio_path, audio_pooling=opt.audio_pooling)
...
for i in range(opt.epoch):
    loss = trainer.train_step(unet_number = 2, max_batch_size = 2, ignore_time = opt.ignore_time)
    print(f'loss: {loss}')

    if not (i % 100):
        valid_loss = trainer.valid_step(unet_number = 2, max_batch_size = 2)
    ...
    if not (i % opt.save_per) and trainer.is_main: # is_main makes sure this can run in distributed
        videos = trainer.sample(text_embeds = aud_emb.unsqueeze(0), video_frames = 10, stop_at_unet_number  = 2, batch_size = 1, cond_video_frames=cond_video_frames.unsqueeze(0))
